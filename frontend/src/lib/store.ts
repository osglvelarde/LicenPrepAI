  ...
  textbooks: [
    {
      ...
      status: 'processed', // now TypeScript knows!
...
type AppState = {
  ...
  actions: {
    ...
    resetPractice: () => void;
    moveMCQToCategory: (mcqId: string, from: keyof AppState['faculty'], to: keyof AppState['faculty']) => void;
    updateSettings: (settings: Partial<AppState['settings']>) => void;
    setQuestions: (questions: MCQ[]) => void;
    addToFacultyPending: (mcqs: MCQ[]) => void;
  };
};
    ...
    (set, get) => ({
      ...
      actions: {
        ...
        moveMCQToCategory: (mcqId, from, to) => set((state) => {
          const mcq = state.faculty[from].find(m => m.id === mcqId);
          if (!mcq) return state;
          return {
            faculty: {
              ...state.faculty,
              [from]: state.faculty[from].filter(m => m.id !== mcqId),
              [to]: [...state.faculty[to], mcq],
            }
          };
        }),
        ...
        addToFacultyPending: (mcqs) => set((state) => ({
          faculty: {
            ...state.faculty,
            pending: [...state.faculty.pending, ...mcqs],
          }
        })),
      }
    }),
    {
      name: 'licenprep-storage', // Persist key
  partialize: (state) => ({
    ...
    faculty: state.faculty,  // Persist faculty review items
  }),
    }
  )
);
