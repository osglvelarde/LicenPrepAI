      ...
      case "complete":
        return (
          <div className="flex gap-2">
            <Badge className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">Complete</Badge>
            {!mimeType.includes('pdf') && (
              <Badge variant="outline" className="bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">OCR Fallback</Badge>
            )}
          </div>
        );
      case "error":
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold tracking-tight">Upload Learning Materials</h1>
      </div>
                  ...
                  <TableRow key={file.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span>{file.file.type || "application/octet-stream"}</span>
                      </div>
                    </TableCell>
              ...
              <TableBody>
                {textbooks.map(book => (
                  <TableRow key={book.id}>
                    ...
                    <TableCell>{new Date(book.uploadedAt).toLocaleDateString()}</TableCell>
                    <TableCell>
                      <Badge variant={book.status === 'processed' ? 'default' : 'outline'}>
                        {book.status === 'processing' ? 'Processing' : 
                         book.status === 'processed' ? 'Ready' : 'Failed'}
                      </Badge>
