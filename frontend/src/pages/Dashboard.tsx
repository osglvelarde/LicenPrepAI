    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {systemProgress.slice(0, 6).map((system) => (
          <Card key={system.system} className="animate-in">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">{system.system}</CardTitle>
            </CardHeader>
              ...
              <div className="relative h-32 w-32">
                <svg className="h-full w-full" viewBox="0 0 100 100">
                  ...
                  <circle
                    ...
                    r="40"
                    fill="transparent"
                    strokeDasharray={`${system.progress * 2.51} ${251 - system.progress * 2.51}`}
                    strokeDashoffset="0"
        ...
        <Card className="animate-in">
          ...
          <CardFooter>
            <p className="text-sm text-muted-foreground">Keep it up! Study every day to maintain your streak.</p>
          </CardFooter>
        </Card>
        ...
        {textbooks.length === 0 ? (
          <Card className="animate-in">
            ...
            <CardContent className="flex flex-col items-center justify-center py-6 space-y-4">
              ...
              <p className="text-center text-muted-foreground">Upload your first textbook to start learning</p>
            </CardContent>
            <CardFooter>
            ...
            <CardContent>
              <ul className="space-y-2">
                {textbooks.map((textbook) => (
                  <li key={textbook.id} className="flex items-center justify-between">
                    <span className="truncate">{textbook.title}</span>
                    <Badge variant={textbook.status === 'processed' ? 'default' : 'outline'}>
                      {textbook.status === 'processing' ? 'Processing' : 
                       textbook.status === 'processed' ? 'Ready' : 'Failed'}
                    </Badge>
                  </li>
                ))}
              </ul>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="bg-card text-card-foreground rounded-full px-3 py-1 text-xs shadow-sm">
                <span className="text-muted-foreground">Latency: 124ms | Tokens: 287</span>
              </div>
            </TooltipTrigger>
            <TooltipContent>
